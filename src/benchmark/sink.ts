...
// Benchmark runs write to run-scoped localStorage keys so they never mix with the
// dogfooding stream (knowdb-traces / knowdb-gaps); a stray dogfooding dump read
// alongside a benchmark dump stays separable by key.
...
// Side-car: append-only JSONL of query_id → variant, joined to the trace dump by
// query_id (the same opaque-id join as GapEvent). Mirrors the trace sink.
